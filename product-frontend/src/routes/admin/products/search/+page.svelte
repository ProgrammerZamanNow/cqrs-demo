<script lang="ts">
	...
	// list/search dibaca dari OpenSearch (read model); tulis ke PostgreSQL (write model)
	const READ = '/api/products/_search';
	...
	async function load() {
		...
		try {
			...
			const res = await fetch(READ + '?' + params.toString());
	...
	// Edit: ambil data terbaru dari write model (PostgreSQL) lewat GET detail.
	...
	async function save(e: Event) {
		...
		try {
			...
			if (res.ok) {
				showForm = false;
				flashNotice('Tersimpan ke PostgreSQL. Akan tampil di OpenSearch ~1 detik setelah CDC sync — klik ↻ Refresh.');
				await load();
			} else {
				formError = json.error ?? `error ${res.status}`;
			}
	...
	async function remove(p: Product) {
		...
		try {
			...
			const json = await res.json().catch(() => ({}));
			if (res.ok) {
				flashNotice('Dihapus dari PostgreSQL. Hilang dari OpenSearch ~1 detik setelah CDC sync — klik ↻ Refresh.');
				await load();
...
<div class="head">
	<h1>Products</h1>
	<span class="src mono">via OpenSearch</span>
	<span class="count mono">{fmt(total)} produk</span>
	<span class="took mono"><i></i>API {took} ms</span>
	<button class="ghost mono" onclick={load} title="Muat ulang dari OpenSearch">↻ Refresh</button>
...
<div class="card list">
	<div class="toolbar">
		<form class="search" onsubmit={applySearch}>
			...
			<input bind:value={keyword} placeholder="Cari produk (n-gram OpenSearch)…" aria-label="Cari produk" />
